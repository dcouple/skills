...
1. It routes state 2 to `simple-plan` instead of `create-plan`.
...
2. `investigating`: use `investigate` when behavior/root cause is unknown.
   ...
   deferred. When any escalation trigger fires, route to `create-plan` instead
   and skip `discussion`: the full lane settles design inside planning and at
   its human approval gate, so running both would duplicate that work.
