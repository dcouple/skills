...
2. `investigating`: use `investigate` when behavior/root cause is unknown.
   ...
   `simple-plan` by default, or to `create-plan` when any escalation trigger
   fires. Run `discussion` before either plan so the open design decision is
   settled and recorded rather than deferred.
