...
- Use **one** primary `/codex:rescue --wait --fresh --model gpt-5.6-sol --effort medium` run for the end-to-end implementation
- Use `--effort xhigh` instead for harder work with fuzzy architecture boundaries or expensive mistakes
- Reserve `max` and `ultra` for incredibly complex, long-running tasks where the extra cost is justified
