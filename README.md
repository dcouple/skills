...
**v3 — Adversarial review via Codex.** The blog post mentions this, but the file layout shows it more clearly: `.codex/skills/` has dedicated `plan-reviewer` and `implementation-reviewer` roles that don't exist on the Claude side. The point is *different model, different prompt, no shared context*. A Claude plan reviewed by a Claude reviewer in the same session is theater. A Codex review with no memory of the original conversation catches the things Claude rationalized.
