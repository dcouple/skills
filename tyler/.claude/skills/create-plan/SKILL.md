---
name: create-plan
description: Creates a reconciled implementation plan by combining a structured plan draft with a normalized intent brief and a PRP-style research dossier, then auto-reviews the final plan. Auto-detects active specs in ./tmp/specs/ and picks the next unchecked phase if no feature description is given. Use when planning a new feature or significant change.
...
3. **In spec mode:**
   ...
   c. **Mark the checkbox immediately**, before continuing, to prevent double-picking if interrupted. Use the Edit tool:
   ...
   e. **The reconciled plan file MUST reference the source spec at the top**, in the `Source Artifacts` section, e.g.:
      ```
      Spec (source of intent): ./tmp/specs/YYYY-MM-DD-name.md (Phase N)
      ```

   f. **The brief / intent artifact (Step 2b) inherits Phase scope.** Treat the spec's Problem, Architectural Decisions, and the phase's Scope/Out-of-scope as the authoritative inputs to the brief — do not re-derive intent from `$ARGUMENTS`.
...
If running in spec mode (Step 0), inherit Problem, Locked Decisions, and
Non-goals from the source spec and the target phase row. Reference the spec
path explicitly at the top of the brief.
...
4. **Present to the user:**
   ...
   **a) Plan Summary** — 3-5 bullet points covering what the plan does. If running in spec mode, name the source spec and phase.
...
- [ ] If spec mode: source spec + phase referenced in `Source Artifacts`, phase checkbox flipped to `[x]`
...
- **Specs (stateful, never moved)**: `./tmp/specs/`
