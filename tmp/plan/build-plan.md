...
- [x] Swap Implementer, Code Reviewer, Plan Reviewer to `codex exec` via the Claude-side **`codex` dispatch skill** (`tyler/.claude/skills/codex/`) — role table pins model/effort/sandbox (implementer: gpt-5.5 medium, workspace-write, `resume --last` across fix loops; reviewers: gpt-5.5 high, read-only, ephemeral). Claude agents remain as fallback (implementer) / dual lane (reviewers).
  - [x] Investigator swap — Codex gpt-5.5 high (workspace-write for tests/repro; edits forbidden by role instructions); Claude Opus agent kept as fallback. Code-researcher also swapped: Codex gpt-5.5 medium read-only, Claude Sonnet fallback.
...
- [x] `.codex/skills/` role skills (implementer, plan-reviewer, code-reviewer) — **thin pointers** into `~/.claude/agents/` charters + `~/.references/` formats, not a full skill mirror: Claude stays the only orchestrator, so `/do` and the workflow skills are not mirrored into Codex.
...
- **2026-07-05 (round 7)** — **Single-copy references (Tyler's direction, informed by Pocock's "context pointer" pattern + Claude Code docs).** Agents can't be folders (flat `.md` only, per official docs), so a new `tyler/references/` tree (synced to `~/.references/`) now holds every doc referenced by >1 skill or by any agent: the shared blocks (verification-criteria, system-analysis — previously duplicated ×3 across the capture skills) and all 7 agent output formats. Agent bodies replace their inlined formats with a Read-pointer plus a short non-negotiables safety net; capture skills point at `~/.references/…`. `tmp/templates/` content moved to the live homes; its README is now the index. Skill-specific doc formats stay in each skill's `references/` (exactly one consumer, so no duplication).
...
- **2026-07-05 (round 9)** — **Clarity + neutral references + researcher/investigator swaps (Tyler's direction).** (1) De-jargoned every runtime file: "Orchestra" and "lane" removed from skills/agents/.codex files — dispatched models get plain definitions ("an automated software-development pipeline conducted by a separate orchestrating agent"); the codename survives only in tmp/ design docs and one defining mention in tyler/README. (2) Shared references moved to a harness-neutral home: repo `tyler/references/` → synced to `~/.references/` (sibling of ~/.claude and ~/.codex); all pointers rewritten. (3) code-researcher and investigator are now Codex-primary — added to the codex skill's role table (researcher: gpt-5.5 medium read-only; investigator: gpt-5.5 high workspace-write) with .codex role skills + openai.yaml; /do, /discussion, /create-issue, /create-feature, /create-epic dispatch them via the codex skill with same-named Claude sub-agents as fallback; dispatching skills got `Skill` added to allowed-tools.
- **2026-07-05 (round 10)** — **GitHub + Notion publish/pull bridge + project templates (Tyler's direction).** New `notion` skill (operations: publish / upload / pull) — Notion is the durable artifact home per work item; uses Notion MCP tools via ToolSearch (no notion CLI exists on this machine; returns NOTION UNAVAILABLE for graceful GitHub+local-only degradation). Capture skills' final step is now "Mark ready and publish": gh issue create → notion publish (item.md + refs/) → cross-link (issue body, item frontmatter `github:`/`notion:`). `/do` accepts a GitHub issue as input (notion pull → ./tmp/<id>/ before work; issue body as fallback item) and uploads plan.md/wrapup.md + PR URL + status done at wrap-up; PR body carries `Closes #<n>`. New `tyler/templates/`: copyable AGENTS.md (universal) + CLAUDE.md (points at AGENTS.md; carries the per-project `Work-item tracking` config — github_repo + notion_data_source — that the skills read).
- **2026-07-05 (round 11)** — **/do slimmed to high-level (Tyler's direction).** ~140 → ~95 lines: the orchestrator judges how much research a plan needs and when review loops have converged (fixed 3-pass caps and the union/dedup mechanics dropped from the text — "until no Must Fix remains from either reviewer"); the standalone clean-up stage and the verify-fix cap removed; PR prep compressed to essentials. /do and all three /create-* skills now carry `disable-model-invocation: true` — user-invoked only.
